	...
	if FLAGS.ae:
		with tf.Session(config=run_config) as sess:
			...
			if FLAGS.train:
				...
				# imae.test(FLAGS)
				hdf5_file = h5py.File(FLAGS.data_dir+'/'+FLAGS.dataset+"_z.hdf5", mode='r')
				z = hdf5_file["zs"][:]
				imae.test_z(FLAGS, z, 64)
	else:
		if FLAGS.train:
			...
			with tf.Session(config=run_config) as sess_z:
				zgan = ZGAN(
						...
						sample_dir=FLAGS.sample_dir,
						data_dir=FLAGS.data_dir)
				generated_z = zgan.get_z(FLAGS, 100)
			...
			with tf.Session(config=run_config) as sess:
				imae = IMAE(
						...
						sample_dir=FLAGS.sample_dir,
						data_dir=FLAGS.data_dir)
				imae.test_z(generated_z, 64, FLAGS.sample_dir)
